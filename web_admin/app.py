...
# Funzioni stub per sostituire le funzionalità quiz rimosse
def carica_quiz():
    """Funzione stub che restituisce una struttura vuota per i quiz."""
    return {"categorie": []}

def carica_statistiche_quiz():
    """Funzione stub che restituisce una struttura vuota per le statistiche dei quiz."""
    return {"partecipanti": {}, "quiz_giornalieri": []}

def load_pending_quizzes():
    """Funzione stub che restituisce una struttura vuota per i quiz in attesa."""
    return {"quiz_pending": []}

def salva_quiz(quiz_data):
    """Funzione stub che non fa nulla."""
    pass

def approve_pending_quiz(index):
    """Funzione stub per approvare un quiz in attesa."""
    return False

def reject_pending_quiz(index):
    """Funzione stub per rifiutare un quiz in attesa."""
    return False

async def invia_quiz_al_canale(context, channel_id, quiz_obj):
    """Funzione stub per inviare un quiz al canale."""
    return False
...
def dashboard():
    try:
        ...
        # Inizializza le variabili per le statistiche dei quiz (funzionalità rimossa)
        total_quizzes = 0
        pending_quizzes = 0
...
def quizzes():
    ...
    flash('La funzionalità dei quiz è stata rimossa dal sistema.', 'warning')
    return redirect(url_for('dashboard'))
...
def approve_pending_quiz_route_impl():
    ...
    app.logger.warning("Tentativo di approvare un quiz, ma la funzionalità è stata rimossa")
    return jsonify({"success": False, "message": "La funzionalità dei quiz è stata rimossa dal sistema."})

@app.route('/quizzes/reject_pending', methods=['POST'])
@login_required
def reject_pending_quiz_route_impl():
    """Rifiuta un quiz in attesa."""
    app.logger.warning("Tentativo di rifiutare un quiz, ma la funzionalità è stata rimossa")
    return jsonify({"success": False, "message": "La funzionalità dei quiz è stata rimossa dal sistema."})
...
# Importa le route per le statistiche
from web_admin.routes.statistiche_routes import *

# Importa le route per le API
from web_admin.routes.api_routes import *
